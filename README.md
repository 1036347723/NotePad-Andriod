...
这是一个记事本应用，已完成时间戳与查找笔记的两个基本功能，增加了在主目录时更改笔记条颜色、编辑笔记时更改背景颜色的两个功能，同时尝试对笔记进行排序，该功能尚未完全实现。
# 1.时间戳功能
...
## 3.2在NotePadProvider的onCreate方法中添加颜色字段，并且在static{}和insert()做相应的处理：
```
 @Override
...
```
## 3.3自定义MyCursorAdapter继承SimpleCursorAdapter
```
...
```
## 4.2在NoteEditor的onResume()方法中增加读取颜色并选择颜色方法。
```
...
```
## 4.3在editor_options_menu.xml中增加一个更改颜色的控件：
```
...
## 4.4在NoteEditor中增加改变颜色的函数changeColor(),之后在onOptionsItemSelected()中增加一个case：
...
```
    case R.id.menu_color:
        ...
        break;：
```
## 4.5新建note_color.xml、colors.xml与NoteColor.java
...
```
## 4.6在AndroidManifest.xml中将这个Acitvity主题定义为对话框样式：
```
